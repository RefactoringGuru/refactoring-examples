...
# Как видите, метод принимает в качестве параметров диапазон из двух даты. Это довольно частая картина и было бы неплохо вместо передачи двух дат передавать объект диапазона дат.
...
# Теперь найдём все места, где вызывается этот метод, и допишем в этих новых вызовах новый параметр, а именно объект, созданных из уже подаваемых в метод дат.
...
# Сперва займёмся параметром <code>start</code>
