...
#V+ Перерассчёты новых значений происходят при потере фокуса элементом. При изменении полей <code>Start</code> или <code>End</code> вычисляется <code>length</code>; при изменении поля <code>length</code> вычисляется <code>End</code>.
...
# Теперь пора взяться за поля. Проведём <a href="/self-encapsulate-field">само-инкапсуляцию</a> каждого из полей окна интервала. Создадим для каждого из полей геттер и сеттер, возвращающий и принимающий строкове значение. Начнем с поля конца интервала:
...
# После чего можно заменить все обращения к <code>endField</code> вызовами соотвествующих методов.
...
# В обратном случае, вызов метода <code>getEnd()</code> привел бы к тому, что при каждом изменении пользователем значения поля, этот код возвращал бы старое значение обратно, поэтому здесь нужно использовать прямой доступ.
...
#C После завершения всех перестановок с полем, можено запустить компиляцию и тестирование.
...
# В итоге мы получим класс предметной области, содержащий все поведения и вычисления над исходными данными, причем отдельно от кода пользовательского интерфейса.
