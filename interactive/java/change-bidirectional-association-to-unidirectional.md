...
Select "double |||getPriceFor|||(Order order) {"

#V Метод получения цены заказа внутри класса покупателя.

+ Select "double |||getDiscountedPrice|||() {"
...
# Другая рассматриваемая альтернатива – такое изменение метода получения данных, которое позволяет ему получать паокупателя, не используя поле. Тогда можно применить к телу <code>Order.getCustomer</code> <a href="/substitute-algorithm">Замещение алгоритма</a> и сделать что-то вроде следующего.
...
# После этого, в методах, которым нужна была ссылка на объект покупателя, прямой доступ к полям нужно заменить вызовами метода <code>getCustomer()</code>
