...
#< ...в котором тип сотрудника задается параметром конструктора.

Select "new Employee"

# Клиентский код вызывает этот конструктор напрямую.

# Предоположим, мы захотели создать подклассы для каждого типа сотрудников.
...
# Но если потом что-то снова поменялось, и мы создали бы еще больше подклассов, вполне возможно, что вызовы конструкторов снова пришлось бы править.
...
# Класс <code>Employee</code> является лучшим метом для хранения фабричного метода, так как он, скорее всего, переживет любые изменения подклассов.
...
# После всех замен, конустор можно скрыть от посторонних глаз, сделав его приватным.
