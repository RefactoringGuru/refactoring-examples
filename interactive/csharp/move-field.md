...
# Стоит упомянуть, что если в классе есть много методов, которые используют перемещаемое поле, то имеет смысл его «самоинкапсулировать» в приватное свойство, чтобы впоследствии упростить рефакторинг. Давайте рассмотрим пример.

Go to "private AccountType type;|||"

# Вернем наше поле и произведем его "самоинкапсуляцию".
